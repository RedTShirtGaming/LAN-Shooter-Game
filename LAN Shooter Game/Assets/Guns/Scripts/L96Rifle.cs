...
public class L96Rifle : MonoBehaviour
{
    ...
    *//*[SerializeField] private Vector2 bulletSpawnPointRotation = new Vector2(1, 1);
    ...
    private void Update()
    {
        ...
        *//*if (Input.GetKeyUp(gunData.fireKey)) 
        ...
        if (Input.GetKeyDown(gunData.firingModeSwitch))
        {
            CycleFiringMode();
        }

        if (Input.GetKeyDown(gunData.altFireKey))
        {
            if (scoped)
            {
        ...
        *//*Quaternion shootPointRotation = new Quaternion(Random.Range(-aimCone, aimCone), Random.Range(-aimCone, aimCone), 0, 0);
        ...
        if (Input.GetKey(gunData.fireKey) && Time.time >= nextTimeToFire)
        {
            ...
            Shoot();
            recoilScript.RecoilFire(recoilX, recoilY, recoilZ, adsRecoilX, adsRecoilY, adsRecoilZ);
        }
    }

    void CycleFiringMode()
    {
        if (gunData.gunFireMode == GunFireMode.Burst) { gunData.gunFireMode = GunFireMode.SemiAutomatic; }
        else if (gunData.gunFireMode == GunFireMode.SemiAutomatic) { gunData.gunFireMode = GunFireMode.Automatic; }
        else if (gunData.gunFireMode == GunFireMode.Automatic) { gunData.gunFireMode = GunFireMode.Burst; }

        print(gunData.gunFireMode);
    }*/
}
